...
- **画像のクリップボード貼り付け（拡張機能側・1/2）**: Webview からの `saveClipboardImage`（`{ base64, mime }`）を受け、編集中ドキュメントと同じフォルダ（`document.uri` 基準）へ一意名で画像を書き込み、`insertImagePath`（`{ path }`＝POSIX区切りの相対パス）を postMessage で返す（`markdownEditor.ts` の `saveClipboardImage`）。保存は既存 `saveMermaidPng` と同方式（`atob`→`Uint8Array`→`vscode.workspace.fs.writeFile`）だがダイアログ無しで自動命名。未保存（untitled／非fileスキーム）は警告して中断、空base64は無視、`readDirectory` 失敗時は衝突チェック無しで続行。純粋ロジックは vscode 非依存の `src/imagePaste.ts` に分離してユニットテスト（`mimeToExtension`＝未知/パラメータ付き/大文字対応・`buildImageFilename`＝`image-<yyyyMMdd-HHmmss>.<ext>`・`disambiguateFilename`＝衝突時 `-1`,`-2`…・`toMarkdownRelativePath`＝`path.relative`＋`/`正規化）。統合テスト（`extension.test.ts`）で実書き込み・別名保存・untitled中断を検証。**Webview側の paste 検出→保存要求→`![](path)` 挿入（2/2）は未実装＝現状このバックエンドは到達不能**で ROADMAP に残置（`src/markdownEditor.ts` 変更＝統合テスト対象）
