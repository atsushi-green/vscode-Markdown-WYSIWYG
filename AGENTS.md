...
- **リアルタイム同期**: WYSIWYGビューとMarkdownソースの双方向同期。Webview→拡張は `edit`（150msデバウンス）、拡張→Webview は `onDidChangeTextDocument`→`update`。**編集中のキャレット保持**: `handleUpdateMessage` の innerHTML 全書き換えでキャレットが飛ばないよう、`utils.saveCursorPosition`/`restoreCursorPosition` が3段のアンカー（①ノード基準＝インプレース変更向け・②ブロック基準＝全書き換え向け・キャレットのトップレベルブロックのインデックス＋ブロック内オフセット・③グローバル文字数オフセット＋最寄りフォールバック）で復元する。**競合する古いエコーの無視**: Webview は `edit` 送信ごとに単調増加する `state.editSeq` を採番（全送信を `editor.js` の `postEdit` へ集約し `seq` を載せる）し、拡張機能は `markdownEditor.ts` でその編集適用に起因するエコー `update` にだけ `seq` を反映（`pendingEchoSeq` を適用前に保持→`updateWebview` で一度きり消費・外部編集には付けない・無変更時は `updateTextDocument` が `false` を返すのでクリア）。Webview は `utils.shouldIgnoreStaleUpdate(message.seq, state.editSeq)` で「エコーの seq が最新送信 seq より小さい＝陳腐化」を無視し、タイプ中に古いエコーが新しいローカル内容を巻き戻すのを防ぐ（`src/markdownEditor.ts` 変更＝統合テスト対象）
