...
window.EditorUtils = (function() {
    ...
    /**
     * 遅れて描画される要素の完了を待つ（出力前の待ち合わせ用）。
     *
     * **現在この関数を呼ぶ本番コードはない。** `window.print()` を使う印刷導線は
     * Webview の sandbox でブロックされることが実機で判明したため取り除いたが、
     * 代替として予定している「HTMLへ書き出して外部ブラウザで開く」方式でも
     * 「図・数式・画像の描画が終わってからHTMLを抜き出す」必要があるため、
     * テスト付きのまま残している（ROADMAP「PDF出力の代替方式」参照）。
     *
     ...
     * これらを待たずに出力（HTMLの抜き出し・印刷）を実行すると、図・式・画像が
     * 欠けた結果になる。
     *
     * **決して reject せず、必ず `timeoutMs` 以内に解決する**。出力は
