...
* **印刷・PDF出力**: `Ctrl+P`（Mac: `Cmd+P`）で印刷ダイアログを開き「PDFとして保存」できます。編集用UI（ツールバー・パンくず・行番号・ダイアログ等）は印刷されず、背景白・本文黒に固定されるためダークテーマのままでも白紙になりません（Rawモード表示中は白紙になるためWYSIWYG表示へ戻してから印刷してください）
