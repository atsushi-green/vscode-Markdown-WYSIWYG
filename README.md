...
* **数式（KaTeX）**: インライン数式（`$E = mc^2$`）とブロック数式（`$$` で囲む）を表示。インライン数式は閉じの `$` を打った時点でその場で即座にレンダリングされます（Raw切替や再読込は不要）。通常のドル記号を書きたいときは `\$100` のようにエスケープします。レンダリング済みのブロック数式（`$$…$$`）を**右クリック**すると「画像としてコピー」でPNG画像としてクリップボードへコピーできます
