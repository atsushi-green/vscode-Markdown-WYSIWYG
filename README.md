...
* **行番号表示**: WYSIWYG・Markdownソース（Raw）どちらの表示でも左端に行番号を表示。番号はMarkdownソースの行番号に対応します（WYSIWYGでは各ブロックの先頭行に番号を表示）。Rawでは行番号を正確に揃えるため長い行は折り返さず横スクロールになります（VS Codeの行折り返しオフと同じ挙動）
