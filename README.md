...
* **目次(TOC)の自動生成**: 見出しから目次を生成して挿入（ツールバーの 📑 ボタン、または `Ctrl+Shift+O`）。GitHub風のアンカーリンク付きで、リンクをクリックすると該当見出しへスクロール
...
* 引用（`> >` によるネスト対応）
* GitHubアラート（`> [!NOTE]` / `[!TIP]` / `[!IMPORTANT]` / `[!WARNING]` / `[!CAUTION]` をタイプ別に色分け表示）
