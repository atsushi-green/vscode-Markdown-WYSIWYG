...
* **数式（KaTeX）**: インライン数式（`$E = mc^2$`）とブロック数式（`$$` で囲む）を表示。インライン数式は閉じの `$` を打った時点で、ブロック数式は閉じの `$$` 行を入力した時点で、その場で即座にレンダリングされます（Raw切替や再読込は不要）。通常のドル記号を書きたいときは `\$100` のようにエスケープします。レンダリング済みのブロック数式（`$$…$$`）を**右クリック**すると「画像としてコピー」でPNG画像としてクリップボードへコピーできます。メニュー上部で背景色（透過／白／黒）と文字色（自動／黒／白）を選べます（既定は背景=白・文字=自動）。文字色「自動」は背景に追従（白・透過→黒、黒→白）し、透過背景をダークな資料へ貼るときは文字色「白」を選べば埋もれません
* **表（Excel風の編集）**: パイプ表（`| A | B |`）をそのまま編集できる表として表示。セル編集・行/列の追加削除・Excelとの相互コピー＆ペースト・ドラッグでの矩形範囲選択（選択範囲だけをコピー・選択範囲の左上を起点に貼り付け可能）に対応。コピーは**タブ区切りテキストと表のHTMLの両方**をクリップボードへ載せるので、Excel ではセルに展開され、Teams など書式を扱える貼り付け先では罫線付きの表として貼り付きます（セル内の太字やリンクも保たれます）。**空行を右クリック**すると「➕ 表を挿入…」から行数・列数を指定して新しい表を挿入できます（入力ダイアログは右クリックした位置の近くに表示されます）
* **画像のクリップボード貼り付け**: スクリーンショットなどの画像をコピーしてエディタに**貼り付け（Ctrl/Cmd+V）**すると、Markdownファイルと同じフォルダへ画像ファイル（`image-<日時>.<拡張子>`）として保存し、その相対パスを `![](…)` としてカーソル位置へ挿入します（Markdownファイルを保存済みの場合。Excel等のテキストを含むコピーは従来どおり表/テキストとして貼り付け）。挿入するパス中の記号（半角スペース・`( )`・`#`・`?` など）は `%20` のようなパーセント記法へ変換します（現在の保存先は同一フォルダ固定・ファイル名も自動生成のため、将来サブフォルダ保存に対応したときにパスが壊れないようにするための予防措置）
* **画像の表示**: `![alt](url)` の画像記法をエディタ内で画像として表示します。Markdownファイルからの**相対パスのローカル画像**（同じフォルダや `assets/…` など）や `data:` URI も実際に描画されます（編集中も元の相対パスを保持し、往復で変わりません）
...
* **行番号表示**: WYSIWYG・Markdownソース（Raw）どちらの表示でも左端に行番号を表示。番号はMarkdownソースの行番号に対応します（WYSIWYGでは各ブロックの先頭行に番号を表示）。Rawでは既定で行番号を正確に揃えるため長い行は折り返さず横スクロールになります（VS Codeの行折り返しオフと同じ挙動）。ツールバーの `↩️ 折り返し` ボタンで折り返しをONにできます（ONの間は行番号が非表示になります）
* **生Markdown表示**: リンク・太字・斜体・取り消し線・下線の内側にカーソルを置いている間だけ、生のMarkdown記法（`[text](url)` / `**太字**` など）を薄く表示し、記法ごとその場で修正可能（カーソルを外すとレンダリング表示へ戻る）。数式はレンダリング済みの式を**クリック**すると生Markdown（`$...$` / `$$...$$`）へ展開して編集でき、外すと再レンダリングされる
...
* **タイトル記法**: `[テキスト](URL "タイトル")` / `![alt](画像URL "タイトル")` のタイトルをツールチップとして表示し、保存時に元の記法へ戻します（`'タイトル'` も可。URLとの間に空白が必要）
* **ツールバーの非表示**: コマンドパレットの「Markdown: ツールバーの表示/非表示を切り替え」または設定 `markdownWysiwyg.showToolbar` で、上部のツールバーを隠して本文の表示領域を広げられます（設定として保存され、再起動後も保たれます）
* **印刷・PDF出力（開発中）**: 出力時のスタイル（編集用UIの非表示・改ページ制御・用紙幅への収まり・ダークテーマ対策）は実装済みですが、**出力を起動する手段はまだありません**。VS Code のエディタ画面（Webview）はブラウザの印刷機能を使えず、`Ctrl+P` / `Cmd+P` もクイックオープンが優先されるため（いずれも実機で確認済み）、HTMLへ書き出して既定のブラウザで開き、そこから PDF 保存する方式を用意する予定です。編集用UI（ツールバー・パンくず・行番号・ダイアログ等）は印刷されず、背景白・本文黒に固定されるためダークテーマのままでも白紙になりません
...
* **「/」コマンドメニュー**: 空の段落で `/` だけを入力すると出るポップアップから「目次を挿入」「表を挿入」を選べる（Notion風のスラッシュコマンド）
* **見出しパンくずバー**: スクロール中、いま見ている位置の見出しを祖先の階層すべてでツールバー直下に表示。クリックで該当見出しへ戻れる
* **脚注**: `本文[^1]` と `[^1]: 脚注の本文` の記法に対応。参照は上付きリンクとして表示され、定義は保存時に文書末尾へ一覧表示されます。参照にマウスを乗せると本文をツールチップ表示、`Ctrl`（Mac: `Cmd`）+クリックで参照⇔定義間を移動できます
* **定義リスト**: `用語` の次行に `: 定義本文` と書くと `<dl>` として表示されます。複数の定義行、複数の用語が定義を共有する形式にも対応
* **YAML front matter の折りたたみ表示**: 文書先頭の `---`〜`---` を折りたたみ可能なブロックとして表示します。ヘッダをクリックすると展開・折りたたみを切り替えられます
...
3. **画像のエクスポート**
   - 図を右クリック → メニュー上部で**背景色（透過／白／黒）**を選び、「画像をコピー」または「PNG画像として保存」
   - 背景色は最後に選んだものが記憶されます（既定は白）。透過を選ぶと背景なしのPNGになります
   - 高解像度（4倍スケール）で保存されます
...
| Raw表示の行折り返し | Raw表示中の行の折り返しを切り替え | `Alt+Z` | `Option+Z` |
...
| 大文字/小文字 | 大文字小文字を区別（検索を開いている間のみ） | `Alt+C` | `Opt+Cmd+C` |
| 単語単位 | 単語単位で検索（検索を開いている間のみ） | `Alt+W` | `Opt+Cmd+W` |
| 正規表現 | 正規表現モード（検索を開いている間のみ） | `Alt+R` | `Opt+Cmd+R` |
...
* テーブルのセル結合など、一部のMarkdown拡張構文は未対応です
* 脚注はサポートしていますが、定義は保存時に文書末尾へまとめられます。段落中の参照は、対応する定義が既にレンダリング済みであれば入力中もその場でリンク表示に変わりますが、見出し・リスト・表・引用中の参照や、参照・定義を両方新規入力した直後は保存/再読込まで見た目が変わりません
