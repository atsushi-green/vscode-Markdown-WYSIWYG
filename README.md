...
* **画像のクリップボード貼り付け**: スクリーンショットなどの画像をコピーしてエディタに**貼り付け（Ctrl/Cmd+V）**すると、Markdownファイルと同じフォルダへ画像ファイル（`image-<日時>.<拡張子>`）として保存し、その相対パスを `![](…)` としてカーソル位置へ挿入します（Markdownファイルを保存済みの場合。Excel等のテキストを含むコピーは従来どおり表/テキストとして貼り付け）
