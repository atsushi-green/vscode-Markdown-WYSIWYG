...
* **表（Excel風の編集）**: パイプ表（`| A | B |`）をそのまま編集できる表として表示。セル編集・行/列の追加削除・Excelとの相互コピー＆ペースト・ドラッグでの矩形範囲選択（選択範囲だけをコピー・選択範囲の左上を起点に貼り付け可能）に対応。**空行を右クリック**すると「➕ 表を挿入…」から行数・列数を指定して新しい表を挿入できます（入力ダイアログは右クリックした位置の近くに表示されます）
