...
* **印刷・PDF出力**: ツールバーの 🖨️ ボタン、またはコマンドパレットの「Markdown: PDFとして出力（印刷）」で印刷ダイアログを開き「PDFとして保存」できます。Rawモード表示中でも自動でWYSIWYG表示へ戻し、図・数式・画像の描画完了を待ってから開きます（**`Ctrl+P` / `Cmd+P` では VS Code のクイックオープンが優先されるため印刷できません**）編集用UI（ツールバー・パンくず・行番号・ダイアログ等）は印刷されず、背景白・本文黒に固定されるためダークテーマのままでも白紙になりません
