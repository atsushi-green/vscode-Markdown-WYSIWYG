...
* **印刷・PDF出力**: ツールバーの 🖨️ ボタン・コマンドパレットの「Markdown: PDFとして出力（印刷）」・`Ctrl+P`（Mac: `Cmd+P`）のいずれかで印刷ダイアログを開き「PDFとして保存」できます。ボタン／コマンドから実行するとRawモード表示中でも自動でWYSIWYG表示へ戻してから印刷します。編集用UI（ツールバー・パンくず・行番号・ダイアログ等）は印刷されず、背景白・本文黒に固定されるためダークテーマのままでも白紙になりません
