...
* **数式（KaTeX）**: インライン数式（`$E = mc^2$`）とブロック数式（`$$` で囲む）を表示。インライン数式は閉じの `$` を打った時点で、ブロック数式は閉じの `$$` 行を入力した時点で、その場で即座にレンダリングされます（Raw切替や再読込は不要）。通常のドル記号を書きたいときは `\$100` のようにエスケープします。レンダリング済みのブロック数式（`$$…$$`）を**右クリック**すると「画像としてコピー」でPNG画像としてクリップボードへコピーできます。メニュー上部で背景色（透過／白／黒）を選べ（既定は白）、黒背景では数式の文字色が自動で白へ反転します
