...
* **印刷・PDF出力（開発中・現在は動作しません）**: 印刷用のスタイル（編集用UIの非表示・改ページ制御・用紙幅への収まり・ダークテーマ対策）は実装済みですが、**出力を起動する手段がまだありません**。`Ctrl+P` / `Cmd+P` は VS Code のクイックオープンが優先され、ツールバーの 🖨️ ボタンとコマンドは Webview がブラウザの印刷機能を使えないため無反応です（実機で確認済み）。HTMLへ書き出して既定のブラウザで開き、そこから PDF 保存する方式を用意する予定です編集用UI（ツールバー・パンくず・行番号・ダイアログ等）は印刷されず、背景白・本文黒に固定されるためダークテーマのままでも白紙になりません
