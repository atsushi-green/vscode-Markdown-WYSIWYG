...
- Rawモード（`#rawEditor` テキストエリア）の左端に、Markdownソースの行番号を表示するガター（`.raw-line-gutter`）を出す。
- 行番号を論理行と1対1で正確に揃えるため、テキストエリアは**行折り返しをオフ**（`white-space: pre`）にしている。長い行は横スクロールで確認する（VS Codeの既定＝`editor.wordWrap: off` と同じ挙動）。
