...
- **強調**: 太字（`**`/`__`)、斜体（`*`/`_`)、太字斜体（`***`/`___`)、下線（`++text++` ⇔ `<u>`）、取り消し線（`~~text~~` ⇔ `<del>`、`<s>`/`<strike>` からの復元にも対応）の相互変換。入力中も記法を打ち切った時点でライブ変換される（contenteditableがテキストを複数ノードに分割してもマッチするよう、走査前に隣接テキストノードを結合している）。
