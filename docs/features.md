...
- **強調**: 太字（`**`/`__`)、斜体（`*`/`_`)、太字斜体（`***`/`___`)、下線（`++text++` ⇔ `<u>`）の相互変換。
- **リンク**: `[text](url)` ⇔ `<a href="url">text</a>`。
- **インラインコード**: `` `code` `` ⇔ `<code>`。
- **引用**: `> ` 行を `<blockquote>` に変換し、連続する引用行は1つの `blockquote` にマージ（複数行対応）。
...
ツールバーからも同じフォーマットコマンド（太字・斜体・見出しH1〜H3・箇条書き・番号付きリスト・リンク・コードブロック・引用挿入）を実行可能。
