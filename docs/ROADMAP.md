...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 編集中に突然キャレットが先頭へ飛ぶ — 根本対策の残り（Mermaid非同期描画完了後の復元・競合updateの無視）※応急処置＋ノード基準（インプレース）＋ブロック基準（全書き換え）は実装済み | M | **これまでに完了**: (a) 応急処置＝復元失敗時に先頭へ飛ばさず最寄りへフォールバック（`cb86e8c`）。(b) **ノード基準アンカー化（インプレース経路）**＝`saveCursorPosition` が `range.endContainer`/`endOffset` を保持し、`restoreCursorPosition` は保存ノードが**テキストノードとしてエディタ内に生きていれば**文字数オフセット走査を経ずにそのノードへ直接復元する（`225ca19`）。(c) **ブロック基準アンカー化（innerHTML全書き換え経路）**＝`saveCursorPosition` が `blockIndex`（キャレットを含むトップレベルブロックのインデックス）/`blockOffset`（ブロック内ローカルオフセット）を保持し、`restoreCursorPosition` はノード基準が使えない全書き換え時に、同インデックスのブロックが残っていればブロック内オフセットで復元する（`(push後に追記)`）。これでキャレットより前の別ブロック（数式・Mermaid・テーブル）が再描画で文字量を変えても、グローバル文字数オフセットのように別ブロックへドリフトしない（ブロック構造が変わった場合は従来のグローバルオフセット走査へフォールバック）。**残る根本原因**: (1) Mermaid の**非同期**再描画（`handleUpdateMessage` の復元は `setTimeout(0)` で、Mermaid の描画完了前に走ると高さ・ノード構成が未確定）— 描画完了後に復元する必要がある。(2) `handleUpdateMessage` の全書き換えを差分適用へ変える／競合する古い `update` を無視する（バージョン番号付与, [markdownEditor.ts:54-66](../src/markdownEditor.ts#L54)）。また、ブロック基準はブロックの**挿入・並べ替え**でインデックスがずれるため、より安定なブロックID（内容ハッシュ等）での対応も検討余地あり。実機（拡張機能開発ホスト）での高速入力・数式/Mermaid編集時の確認が前提 |
