...
| todo | TOCで生成した目次のリンク（`[見出し](#slug)`）をクリックしても該当見出しへ遷移しないバグの修正 | S | 見出しレンダリング側（`media/modules/markdown.js` の `<h${level}>` 生成部、約288行目）が `id` 属性を付与していないため、TOCのアンカーリンクの遷移先が存在しないことが原因と見られる。`buildTocMarkdown`/`slugify`と同じスラッグ生成・重複連番規則（`-1`,`-2`...）を見出しレンダリング側にも適用し、生成した`id`をヘッダー要素に付与する対応が必要 |
...
| todo | ダークモードで太字が視認しにくい問題の改善 | S | `media/editor.css` の `#editor strong { font-weight: 600; }` が原因と見られる。600（セミボールド）は文字色を変えずウェイトのみの違いのため、特にダークモード（暗い背景に明るい文字）ではアンチエイリアスの効き方の違いから通常文字との差が分かりにくい。`font-weight` を700〜800へ引き上げる、または太字部分にわずかな明度・色の強調を加える等の対応を検討 |
