...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | ダークモードで太字がまだ視認しにくい（フォントサイズも少し大きくする） | S | ユーザー再報告: `fd898ed` でウェイトを 600→700（ダーク系は 800）へ引き上げたが、**まだダークモードだと分かりにくい**。仕様（ユーザー指定）: ウェイトに加えて**太字のフォントサイズを少しだけ大きくする**。`media/editor.css` の `#editor strong` と、既存のテーマ別ルール（body の `vscode-dark`/`vscode-high-contrast` クラスで判定・`mermaid.js` と同規則）に `font-size` を追加する。**要検討**: (1) 少しだけ＝`1.05em` 程度から試す（`em` にすると見出し内の太字も相対的に拡大されて自然。`px` 固定は見出しで破綻する）、(2) 明るいテーマにも同じ拡大を適用するか、ダーク系のみにするか（報告はダークモードについてだが、テーマで太字の大きさが変わるのは一貫性を欠くため、全テーマ適用が無難か）、(3) 行内で `font-size` を変えると行の高さが変動して前後の行がわずかにずれるため、`line-height` を固定して見た目が跳ねないか実機で確認する。**実機（拡張機能開発ホスト）でダーク／ライト両方の見え方を確認すること**（CSSのみの変更でユニットテストでは判断できないため） |
