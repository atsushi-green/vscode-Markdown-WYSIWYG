...
| todo | 数式の内側にカーソルがある間、生Markdownを表示する | M | ユーザー要望: レンダリング済みの数式にカーソルを持っていくと生のMarkdown（`$...$` / `$$...$$`）が表示され、その場で編集できるようにする。数式レンダリング（`PENDING` で実装済み）が前提。**現状、数式コンテナは `contenteditable="false"` のためキャレットが内側に入らない**（KaTeXの生成DOMが編集で壊れるのを防ぐため）。この項目ではクリック等でコンテナを選択した際に生Markdownへ展開する形になるため、リンクのようなキャレット所属判定だけでは足りず、`contenteditable` の切り替えを含めた設計が要る。リンクで実装済みの `commands.syncRawMarkdownToCaret`（`selectionchange` 監視・`span.raw-markdown` への展開／復帰・`utils.shouldSkipInline` による再変換抑止）を**汎用化して共有する**のが本命。ブロック数式は複数行になるため、インライン（span）と同じ展開／復帰でよいかは要検討。強調記法の項目とも仕組みを共有できる見込み。実機での見た目・カーソル挙動の確認が必要なため対話セッション向き |
...
| todo | 強調記法（`~~`/`**`/`++`）の内側にカーソルがある間、生Markdownを表示する | M | 取り消し線(`~~`)・太字(`**`)・下線(`++`)・斜体(`*`/`_`)等で囲まれた範囲の**内側のどこかにカーソルがある間ずっと**、生のMarkdown記法を表示して記法ごと修正しやすくする（境界だけでなく範囲内全体が対象）。あわせて、次に入力する文字を装飾の内側/外側どちらに含めたいかをユーザーが直感的に示せるようにする（内側にカーソルがあれば装飾を継続、外側へ置き直せば装飾から抜ける）。カーソルが範囲から外れたら装飾表示へ戻す。上のリンクの生Markdown表示と同じキャレット所属判定・展開/復帰の仕組みを共有できる見込み。実機での見た目・カーソル挙動の確認が必要なため対話セッション向き |
...
| 完了日 | 機能 | コミット |
|--------|------|----------|
| 2026-07-17 | 数式（インライン `$...$` / ブロック `$$...$$`）の表示・双方向変換（KaTeXを `media/katex/` へ同梱・生の式は `data-math` に保持・描画は `math.js`・`\$` エスケープ対応。ユーザー要望） | `PENDING` |
