...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 編集中に突然キャレットが飛ぶ — 競合updateエコー無視の残り（拡張機能側のseqエコー配線 2/2）＋Mermaid非同期描画完了後の復元 | M | **これまでに完了**: (a) 応急処置＝復元失敗時に先頭へ飛ばさず最寄りへフォールバック（`cb86e8c`）。(b) **ノード基準アンカー化（インプレース経路）**（`225ca19`）。(c) **ブロック基準アンカー化（innerHTML全書き換え経路）**＝`saveCursorPosition` が `blockIndex`/`blockOffset` を保持し全書き換え時にブロック内オフセットで復元（`132bd22`）。(d) **競合updateエコー無視のWebview側インフラ(1/2)**＝Webview が編集送信ごとに単調増加 `state.editSeq` を採番（全送信を `postEdit` へ集約して `seq` を載せる）、`utils.shouldIgnoreStaleUpdate(messageSeq, editSeq)` で「エコーの seq が最新送信 seq より小さければ無視」を `handleUpdateMessage` 先頭ガードに配線。純粋関数＋ユニットテスト済み（`(push後に追記)`）。**残る根本原因**: (1) **競合updateエコー無視の(2/2)＝拡張機能側の配線**：`src/markdownEditor.ts` の `updateWebview`（[markdownEditor.ts:54-66](../src/markdownEditor.ts#L54)）で、Webview から受けた `edit` の `seq`（[markdownEditor.ts:76-78](../src/markdownEditor.ts#L76)）を、その編集適用に起因するエコー `update` に `seq` として反映して返す（外部編集＝Webview起因でない変更には `seq` を付けない＝常に適用）。案: `edit` 適用直前に `pendingEchoSeq = e.seq` を保持→`updateWebview` で `seq: pendingEchoSeq` を載せて消費（次の外部変更では undefined）。`src/markdownEditor.ts` を触るため統合テスト対象サイクル。これで(1/2)のガードが実効化する。(2) Mermaid の**非同期**再描画（`handleUpdateMessage` の復元は `setTimeout(0)` で描画完了前に走ると高さ・ノード構成が未確定）— 描画完了後に復元する（ただしブロック基準アンカー導入でドリフトは大幅緩和済み）。また、ブロック基準はブロックの**挿入・並べ替え**でインデックスがずれるため、より安定なブロックID（内容ハッシュ等）も検討余地あり。実機（拡張機能開発ホスト）での高速入力・数式/Mermaid編集時の確認が前提 |
