...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | PDFエクスポート (1/4): 印刷用スタイル（`@media print`）の整備 | S | `media/editor.css` に `@media print` ブロックを新設し、印刷時に**本文だけが残る**ようにする。隠す対象: `.toolbar`（ツールバー全体）・`.heading-breadcrumb-bar`（パンくず）・`.raw-line-gutter`（行番号ガター）・`#rawEditor`（Rawモードのtextarea）・ステータスバー（`.word-count-status`。`position: fixed` なので印刷時に本文へ重なる）・トースト（`.mermaid-toast`）・各種ダイアログ（`.link-dialog`/`.table-insert-dialog`）・コンテキストメニュー（`.mermaid-context-menu`/`.math-context-menu`）・表ツールバー（`.table-toolbar`）・コードブロックのコピーボタンと言語セレクタ。`#editor` は `contenteditable` の枠線・背景・高さ制限（`overflow`/`height`）を解除して全文が流れるようにする。色は VS Code のテーマ変数を使っているため、印刷時は背景を白・本文色を黒へ固定する必要がある（現状 `@media print` は1つも無い）。ただし**無条件に `color: #000` を当てるとコードのシンタックスハイライトが潰れる**（`#editor .hljs-*` やダークテーマ専用ルール `body.vscode-dark #editor strong` が効いている前提）ため、適用範囲は本文の地の色に絞る。**この1件だけで「Ctrl+P → PDFとして保存」が実用になるのは WYSIWYG 表示中のみ**（Rawモード中は `#editor` がインラインスタイルで `display:none` のため、`#rawEditor` を隠すと白紙が印刷される。その救済は (2/4) の「Rawモードは WYSIWYG へ戻してから印刷」に依存する）。優先度は最も高い |
| todo | PDFエクスポート (2/4): 「PDFとして出力」コマンド／ツールバーボタンの追加 | S | (1/4) の印刷スタイルができたら、`window.print()` を呼ぶ導線を用意する。Webview 内では `window.print()` がそのまま使える（拡張機能側からは `webview.postMessage` で`print` を送って `editor.js` 側で呼ぶ形）。ツールバーへ 🖨️ ボタンを追加（`markdownEditor.ts` のツールバーHTMLと `editor.js` の配線。`#toggleRawWrap` 追加時と同じ手順）し、あわせて`package.json` にコマンド（例: `markdownWysiwyg.exportPdf`）を登録する。**Rawモード表示中は WYSIWYG へ戻してから印刷する**（Rawのtextareaは印刷対象外のため）。`src/markdownEditor.ts` を変更するので統合テスト（`npm test`）の実行対象になる |
| todo | PDFエクスポート (3/4): 改ページ制御と用紙幅の調整 | M | コードブロック・表・Mermaid図・ブロック数式・画像が**ページ境界で分断されない**よう `break-inside: avoid`（旧 `page-break-inside`）を当てる。見出しの直後で改ページされないよう `h1〜h6 { break-after: avoid }`、`h1 { break-before: page }` を入れるかは要検討（章ごとに改ページするか）。横幅は画面の可変幅から用紙幅（A4想定）へ切り替わるため、`max-width` の指定と、長い表・コードブロックが用紙からはみ出す場合の折り返し（`overflow-wrap` / 表の `table-layout`）も要調整。`@page { margin: … }` で余白も指定する。**Mermaid図はテーマ追随で描画される点にも対処が要る**——`mermaid.js` の `resolveTheme()` が VS Code のテーマに合わせて初期化するため、ダークテーマ環境では明色の線・文字のSVGになり、印刷時の白背景と合わせると**白紙に白い図**になる。印刷前にライトテーマで再描画するか、SVGの色を印刷用に固定する |
| todo | PDFエクスポート (4/4): 非同期レンダリング完了を待ってから印刷する | M | Mermaid図（`mermaid.js`）・数式（KaTeX）・ローカル画像の解決（`resolveLocalImages`）は非同期に描画されるため、描画途中で `window.print()` を呼ぶと**図や式が欠けたPDF**になる。印刷前に描画完了を待つ制御が要る（Mermaidは `render` の Promise、KaTeXは `document.fonts.ready`、画像は `img.decode()`／`complete` の待機。ブロック数式のPNG化で使っている `restoreCaretAfterRender` と同種の待ち合わせ）。`beforeprint` イベントは同期的にしか待てないため、ボタン経由で「待ってから `window.print()`」する形にする |
