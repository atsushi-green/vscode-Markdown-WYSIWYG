...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | ブロック数式コピー画像の崩れ修正 — blockToPngBlob の foreignObject 差し替え＋フォント埋め込みの**配線**（残り・実機確認前提） | M | **分割の残り配線。純粋コアは実装済み**: (1/2) `math.buildMathBlockSvgMarkup`（SVG/foreignObject 組み立て・背景色対応, `36e6631`）＋フォント埋め込みコア `math.buildKatexFontFaceCss(fonts)`／`math.KATEX_FONT_MANIFEST`（base64 化 woff2 から `@font-face(src: url(data:font/woff2;base64,…))` を組み立てる純粋関数と、埋め込む20フォントの静的一覧＝`katex.min.css` の `@font-face` と一致することをテストで保証, `(未付与)`）。**残り＝ランタイム配線**: `blockToPngBlob`（[math.js](../media/modules/math.js)）を html2canvas から foreignObject SVG 方式へ差し替える: (a) クローンの実寸を `getBoundingClientRect` で採寸、(b) `KATEX_FONT_MANIFEST` の各 woff2 を webview URI から fetch→`ArrayBuffer`→base64 化し `buildKatexFontFaceCss` で `@font-face` CSS を得る（初回のみ・キャッシュ推奨）、(c) KaTeX の `<style>`＋(b) を `cssText` に渡して `buildMathBlockSvgMarkup` でSVG文字列化、(d) `new Image()` の `src` に `data:image/svg+xml`（CSPの `img-src … data:` は許可済み）→canvas描画→PNG Blob。**CSP変更が必須**: フォントを `data:` で埋め込むため CSP へ `font-src … data:` を追加する（現状 `font-src ${webview.cspSource}` のみ, [markdownEditor.ts:172-177](../src/markdownEditor.ts#L172)。`src/markdownEditor.ts` を触るため統合テスト対象サイクル）。**この配線と Image/canvas ラスタライズ・fetch は jsdom で検証不可**なので、見た目・クリップボード貼り付けは実機（拡張機能開発ホスト）確認が必須。あわせて数式の背景色選択（[[Mermaid図の画像コピー／保存時に背景色を選べるようにする（透過・白・黒）]] と同趣旨・現状 `#ffffff` 固定）も `buildMathBlockSvgMarkup` の `background` 引数経由で同時対応できる |
