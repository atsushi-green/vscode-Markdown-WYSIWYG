...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 透過背景でコピーした数式画像がダーク環境に貼ると黒文字で見えにくい | S | ブロック数式のPNGコピーは背景トグルで「透過」を選べる（`math.js`）が、数式の文字色は白・透過背景では黒固定（`blockToPngBlob` の `wrapper.style.color = '#000000'`）。透過PNGをダークな資料/チャットへ貼ると黒文字が背景に埋もれる。黒背景時は白文字へ反転済み（`d?`）だが透過時の文字色は未対応。方向性の候補: (1)透過選択時に文字色（白/黒）も選べるサブトグルを足す、(2)透過時は既定を「currentColor 継承のまま＝画面テーマ依存」にする等。Mermaidは図自身が色を持つため無関係。`/local-review` の B-1 指摘由来 |
