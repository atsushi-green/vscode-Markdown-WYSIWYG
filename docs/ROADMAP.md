...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | ローカル画像記法 `![](path)` のインライン表示（WYSIWYGで画像を描画） | M | 現状 `markdown.js` は画像記法を扱わず（`convertInline` は `[text](url)` リンクのみ、`![](path)` はリテラルテキスト表示）、クリップボード貼り付けで入れた `![](image-….png)` もソースのまま出る。これをエディタ内で `<img>` として表示する。**要点**: (1) `markdown.js` の `convertInline` に画像記法（`![alt](url)`）→`<img src alt>` を追加し、`htmlToMarkdown`（`serializeInline`）で `<img>`→`![alt](url)` へ戻す往復対応（リンク `[text](url)` より**先に**画像を処理し、`![` が `!`＋リンクに割れないようにする）、(2) **CSP**: ローカル画像を表示するには `img-src` に `${webview.cspSource}` に加えて画像ファイルのある場所を許可する必要があり、Webview の `localResourceRoots` はドキュメントのフォルダを含まない（現状 `extensionUri` のみ）ため、`resolveCustomTextEditor` で **ドキュメントのフォルダを `localResourceRoots` に追加**し、`src` を `webview.asWebviewUri(...)` で変換する仕組みが要る（相対パス→webview URI 解決を拡張機能側 or Webview側で行う設計）。`src/markdownEditor.ts` を触るため統合テスト対象。外部URL画像（`https://…`）は別途 CSP 検討（[[シールドバッジ画像の表示]] と関連）。実機で表示・往復・CSPを確認 |
