...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 編集中に突然キャレットが飛ぶ — 残り（Mermaid非同期描画完了後の復元・ブロックID安定化）※応急処置＋3段アンカー＋競合エコー無視は実装済み | M | **これまでに完了**: (a) 応急処置＝復元失敗時に先頭へ飛ばさず最寄りへフォールバック（`cb86e8c`）。(b) **ノード基準アンカー化（インプレース経路）**（`225ca19`）。(c) **ブロック基準アンカー化（innerHTML全書き換え経路）**（`132bd22`）。(d) **競合updateエコー無視**＝Webview が `edit` 送信ごとに `state.editSeq` を採番（`postEdit` へ集約）→拡張機能が起因エコー `update` にだけ `seq` を反映（`pendingEchoSeq`）→Webview が `utils.shouldIgnoreStaleUpdate` で陳腐化エコーを無視。(1/2 `e60bee2`＝Webview側インフラ、2/2 `1aca68d`＝拡張機能側配線で実効化・統合テスト9件パス）。**残る根本原因**: (1) Mermaid の**非同期**再描画（`handleUpdateMessage` の復元は `setTimeout(0)` で描画完了前に走ると高さ・ノード構成が未確定）— 描画完了後に復元する（ただしブロック基準アンカー＋競合エコー無視でドリフト/巻き戻りは大幅緩和済みのため優先度は下がった）。(2) ブロック基準アンカーはブロックの**挿入・並べ替え**でインデックスがずれるため、より安定なブロックID（内容ハッシュ等）での対応も検討余地あり。実機（拡張機能開発ホスト）での高速入力・数式/Mermaid編集時の確認が前提 |
