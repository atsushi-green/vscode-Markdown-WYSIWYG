...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 編集中に突然キャレットが飛ぶ — 残り（Mermaid非同期描画完了後の復元）※応急処置＋3段アンカー（署名によるindexずれ耐性込み）＋競合エコー無視は実装済み・優先度低め | M | **これまでに完了**: (a) 応急処置＝復元失敗時に先頭へ飛ばさず最寄りへフォールバック（`cb86e8c`）。(b) **ノード基準アンカー化（インプレース経路）**（`225ca19`）。(c) **ブロック基準アンカー化（innerHTML全書き換え経路）**（`132bd22`）＋**署名によるindexずれ耐性**＝ブロックの挿入・並べ替えで `blockIndex` がずれても署名（先頭テキスト・`utils.blockSignatureOf`）が一意一致すればそのブロックを辿る（`(push後に追記)`）。(d) **競合updateエコー無視**（1/2 `e60bee2`＋2/2 `1aca68d`・統合テスト9件パス）。**残る根本原因**: Mermaid の**非同期**再描画（`handleUpdateMessage` の復元は `setTimeout(0)` で描画完了前に走ると高さ・ノード構成が未確定）— 描画完了後に復元する（`mermaidModule.render()` は Promise を返すので await 可能）。ただしノード基準＋ブロック基準（署名込み）＋競合エコー無視でドリフト/巻き戻りは**大幅緩和済み**のため優先度は低め。実機（拡張機能開発ホスト）での高速入力・数式/Mermaid編集時の確認が前提 |
