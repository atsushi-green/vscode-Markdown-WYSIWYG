...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | ローカル画像のインライン表示(2/2) — 相対パス `![](path)` を実画像として描画（CSP/localResourceRoots/baseUri） | M | (1/2, `c1007b2`) で **Markdown⇔HTML の画像記法対応は完了**（`convertInline`/`convertInlineText` が `![alt](url)`→`<img src alt>`、`serializeInline` が `<img>`→`![alt](url)`。`data-original-src` があればそれを優先して往復）。**残り(2/2)＝ローカル相対パス画像の実表示**: 現状 `<img src="image-1.png">` は webview のベースURIが拡張機能のため解決されず**壊れた画像アイコン**になる。表示するには (a) `resolveCustomTextEditor` で **ドキュメントのフォルダを `webview.options.localResourceRoots` に追加**、(b) そのフォルダの webview URI（`webview.asWebviewUri(dirUri)`）を init/update で Webview へ渡す、(c) Webview 側で描画後に相対 `<img src>` を baseUri で解決し**元パスを `data-original-src` に退避**して `src` を webview URI へ差し替える（往復は `serializeInline` が `data-original-src` を読むため保たれる＝実装済み）。CSP `img-src` は既に `${webview.cspSource}` を許可済み。`src/markdownEditor.ts` を触るため統合テスト対象。外部URL画像（`https://…`）は別途 CSP 検討（[[シールドバッジ画像の表示]] と関連）。実機で表示・往復・CSPを確認 |
