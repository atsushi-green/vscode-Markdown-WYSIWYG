...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | リンク上にカーソルがある間、生Markdownを薄く表示する | M | **カーソルがリンク（`<a>`）の内側のどこかにある間ずっと**、変換後のリンク表示ではなく元の生Markdown記法（`[text](url)`）を薄く表示し、URLやリンクテキストを直接修正できるようにする（境界だけでなく、リンク文字列のどの位置にカーソルが来ても表示する）。カーソルがリンクから外れたらレンダリング表示へ戻す。キャレット位置監視（`selectionchange`）でカーソルの所属要素を判定し、対象リンクだけを生Markdownテキストへ展開／復帰させる仕組みが必要。展開中のテキストは `applyInlineFormatting` に再変換されないよう抑止する（カーソルが外れた時点で確定・再レンダリング）。下の強調記法の項目と実装方式を共有できる見込み。実機での見た目・カーソル挙動の確認が必要なため対話セッション向き |
