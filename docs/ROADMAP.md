...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | 行番号表示 (1/3): Rawモード（`#rawEditor` textarea）の行番号ガター | S | 分割元: 旧「行番号を表示する」L項目（`44b4031` 時点で3分割）。Rawモードは textarea なので行番号は素直に出せる（`\n` の数＝行数）。textarea の左へ固定幅のガター要素を置き、1〜N行の番号を並べる。textarea のスクロールに合わせてガターを縦スクロール同期（`rawEditor` の `scroll` で `gutter.scrollTop = rawEditor.scrollTop`）、font-family / font-size / line-height / 上下 padding を textarea と厳密一致させる（ずれると番号が本文行とずれる）。行数計算は純粋関数（`countLines(text)` = 末尾改行の扱いに注意）としてユニットテスト。折り返し行に番号を出さない挙動（VS Code準拠）はまず textarea を `white-space: pre`（横スクロール）にして単純化するか、`pre-wrap` のまま開始行のみ出すかを実装時に決める。UIは動的生成で `markdownEditor.ts` を触らない方針を優先（統合テスト回避）。表示の常時ON/切替可否は初期は常時表示でよい。依存なし・単独で着手可 |
