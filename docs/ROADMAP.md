...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | ダークモードで太字がまだ視認しにくい（フォントサイズも少し大きくする） | S | ユーザー再報告: `fd898ed` でウェイトを 600→700（ダーク系は 800）へ引き上げたが、**まだダークモードだと分かりにくい**。仕様（ユーザー指定）: ウェイトに加えて**太字のフォントサイズを少しだけ大きくする**。**拡大は全テーマに適用する**（ユーザー確定。報告はダークモードについてだが、テーマによって太字の大きさが変わるのは一貫性を欠くため）。したがって `media/editor.css` の `#editor strong` に `font-size` を足せばよく、既存のテーマ別ルール（body の `vscode-dark`/`vscode-high-contrast` クラスで判定・`mermaid.js` と同規則）は**ウェイトの出し分けのまま据え置く**。**要検討**: (1) 少しだけ＝`1.05em` 程度から試す（`em` にすると見出し内の太字も相対的に拡大されて自然。`px` 固定は見出しで破綻する）、(2) 行内で `font-size` を変えると行の高さが変動して前後の行がわずかにずれるため、`line-height` を固定して見た目が跳ねないか実機で確認する。**実機（拡張機能開発ホスト）でダーク／ライト両方の見え方を確認すること**（CSSのみの変更でユニットテストでは判断できないため） |
