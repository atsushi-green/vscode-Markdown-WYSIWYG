...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | ツールバーの 🖨️ PDF ボタンとコマンド「PDFとして出力」が押しても何も起きない | S | **実機確認済み**: `Ctrl+P` だけでなく **🖨️ ボタン／コマンドパレット経由でも無反応**だった。VS Code の Webview は sandbox 付き iframe で動くため `window.print()` がブロックされている（コンソールに警告が出るだけで何も起きない）。つまり印刷ダイアログ方式そのものが使えない。**利用者から見れば「押しても何も起きないボタン」＝不具合**なので、代替方式（下記「HTMLへ書き出して外部ブラウザで開く」）が入るまでの間、(a) ボタンとコマンドを一旦外す か (b) 押したときに「この方法では出力できません」と案内するトーストを出す、のどちらかにする。代替方式が入ったらボタンはその導線に付け替える。`window.print()` を呼ぶ `exportPdf`（`editor.js`）と `postToActivePanel`（`markdownEditor.ts`）も整理対象。**なお (1/4)〜(4/4) で作った `@media print` のスタイルと描画待ち合わせは代替方式でも流用できる** |

## 優先度: 中

| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | ツールバーを非表示にできるようにする | S | **利用者からの要望**。上部のツールバー（`.toolbar`）を隠して本文の表示領域を広げられるようにする。実装: `markdown-wysiwyg-editor.toggleToolbar` コマンド＋`package.json` の `configuration` に設定（例 `markdownWysiwyg.showToolbar`、既定 true）を追加し、`editor.js` が設定値に応じて `.toolbar` の表示を切り替える。**セッション限りではなく設定として永続化する**（`isRawMode` などの非永続フラグとは違い「常に隠しておきたい」という要望のはずのため）。見出しパンくずバー（`.heading-breadcrumb-bar`）はツールバー直下に置かれているので、隠したときにレイアウトが崩れないか確認が要る。印刷時は `@media print` が既にツールバーを隠している |
| todo | ツールバーを隠しても操作に困らないよう、キーボード／コマンドの導線を揃える | S | 上の「ツールバーを非表示にできるようにする」とセット。ツールバーにしか無い操作（Raw表示切替 `#toggleView`・行折り返し `#toggleRawWrap`・目次挿入・PDF出力・書式ボタン各種）は、隠した状態では実行手段が無くなる。Raw切替は `Ctrl+/`、目次は `Ctrl+Shift+O` があるが、**行折り返しトグルにはショートカットもコマンドも無い**。不足しているものをコマンドパレットへ登録し、`docs/commands-and-shortcuts.md` の表も更新する |

## 優先度: 低

| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | PDF出力の代替方式: HTMLへ書き出して外部ブラウザで開く | M | `window.print()` が Webview の sandbox でブロックされることが実機で判明したため、印刷ダイアログ方式に代わる実現手段。**依存を増やさずに済む案**: 本文のクリーンHTMLに (1/4)〜(3/4) で作った `@media print` 相当のスタイルと KaTeX/highlight.js のCSSを**インラインで埋め込んだ**自己完結のHTMLを一時ファイルへ書き出し、`vscode.env.openExternal` で既定ブラウザに開く。**ブラウザ上では `Ctrl+P` が普通に効く**ので、利用者はそこから「PDFとして保存」できる。Mermaid図は描画済みSVGをそのまま埋められる（`html2canvas` 経由のPNG化は不要）。画像はローカル相対パスを絶対パス（`file:`）か data URI へ変換する必要がある。`getCleanHtmlFromEditor` と Mermaid のPNG化で確立した「クリーンHTML抽出」の流れを流用できる |
...
| todo | 印刷ボタンの二重クリックで待ち合わせと `window.print()` が二重に走る | S | 描画完了の待ち合わせ（最大5秒）が入ったことで、待機中に🖨️ボタンを再クリックすると`waitForRenderComplete` と `print()` が二重に走る余地ができた。Rawモードから実行した場合は `afterprint` の復帰リスナも二重登録され（`{once:true}` でも）「Raw→WYSIWYG→Raw」の余計なトグルが起きうる。待機中はボタンを無効化する（`disabled` 属性＋進行中フラグ）と良い。`/local-review` C-3 由来（severity low, 確信度低） |
| todo | 印刷前の画像待ちが「クリック時点のスナップショット」になっている | S | `exportPdf` は `state.editor.querySelectorAll('img')` を待ち合わせ開始時に1度だけ取るため、待機中に受信更新やMermaid描画で新しい `img` が挿入されても待たない。現状の使い方では影響は小さいが、Mermaid描画の完了後にもう一度画像を集め直す等の対応が考えられる。`/local-review` B-3 由来（severity low） |
