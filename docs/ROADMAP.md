...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | 画像のクリップボード貼り付け(1/2) — 拡張機能側: 画像バイト列を受けてドキュメント隣へ保存し相対パスを返す | M | 「クリップボードの画像を貼り付け→ファイル保存＋相対パス挿入」のL項目を分割した前半（拡張機能側＝統合テスト対象）。Webview から新メッセージ `saveClipboardImage`（`{ base64, mime }`）を受け取り、**現在編集中ドキュメントと同じフォルダ**（`document.uri` を基準）へ一意なファイル名で書き込み、Webview へ `insertImagePath`（`{ path }`＝挿入用の相対パス・POSIX区切り）を postMessage で返す。実装の要点: (a) `src/markdownEditor.ts` の `resolveCustomTextEditor` は `document` を閉じ込めているのでそれを基準にできる、(b) 保存処理は既存 `saveMermaidPng` の `atob`→`Uint8Array`→`vscode.workspace.fs.writeFile` を流用（ただしダイアログは出さず自動命名）、(c) **純粋関数として切り出してユニットテスト可能にする**: 「mime→拡張子」（`image/png`→`png`・`image/jpeg`→`jpg` 等）、「ファイル名生成」（`image-<yyyyMMdd-HHmmss>.<ext>`）、「ドキュメントURIからの相対パス算出＋衝突回避サフィックス（`-1`,`-2`…）」。**注意**: `src/markdownEditor.ts` を触るため統合テスト対象サイクル。実機でファイルが実際に書かれる確認が要る。(2/2)がこのメッセージ契約に依存 |
| todo | 画像のクリップボード貼り付け(2/2) — Webview側: paste で画像検出→保存要求→返った相対パスで `![](path)` を挿入 | M | 上記(1/2)のメッセージ契約に依存する後半（Webview側）。`media/modules/` で `paste` イベントの `clipboardData.items` から `image/*` を検出し、`FileReader`/`ArrayBuffer`→base64 化して拡張機能へ `saveClipboardImage` を postMessage、返信 `insertImagePath` を受けてキャレット位置へ `![](相対パス)` を挿入する（挿入は既存 `commands.insertToc`/`table.insertTable` のキャレット挿入パターンを流用）。**純粋関数でユニットテスト**: 「clipboardData.items から画像アイテム抽出」「mime→拡張子」「`![](path)` markdown組み立て（alt空・パスのスペースは `%20` か `<...>` 表記）」。**事前確認（往復）**: 画像記法 `![alt](url)` が現状 `markdown.js` で `<img>`⇔`![]()` 双方向に保たれるか（レンダリングされるか）を先に確認し、未対応なら最小の画像記法対応から。`clipboardData`/`FileReader` は実機依存のため**実機（拡張機能開発ホスト）で貼り付け→保存→挿入→往復を確認**。`media/` のみなら統合テスト非対象だが、(1/2)と合わせて実機確認前提 |
