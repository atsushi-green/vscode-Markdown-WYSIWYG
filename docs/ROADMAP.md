...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | 画像のクリップボード貼り付け(2/2) — Webview側: paste で画像検出→保存要求→返った相対パスで `![](path)` を挿入 | M | (1/2, `(pending)`) で拡張機能側は実装済み＝**メッセージ契約は確定している**: Webview → 拡張機能 `saveClipboardImage`（`{ base64, mime }`）を送ると、ドキュメント隣へ一意名（`image-<yyyyMMdd-HHmmss>.<ext>`・衝突時 `-1`,`-2`…）で保存され、拡張機能 → Webview `insertImagePath`（`{ path }`＝POSIX区切りの相対パス）が返る（未保存ドキュメントは警告して何もしない）。本(2/2)は**Webview側のみ**: `media/modules/`（または `editor.js` の paste ハンドラ）で `paste` イベントの `clipboardData.items` から `image/*` を検出し、`FileReader`/`ArrayBuffer`→base64 化して `saveClipboardImage` を postMessage、返信 `insertImagePath` を受けてキャレット位置へ `![](相対パス)` を挿入する（挿入は既存 `commands.insertToc`/`table.insertTable` のキャレット挿入パターンを流用）。**純粋関数でユニットテスト**: 「clipboardData.items から画像アイテム抽出」「`![](path)` markdown組み立て（alt空・パスのスペースは `%20` か `<...>` 表記）」。**事前確認（往復）**: 画像記法 `![alt](url)` が現状 `markdown.js` で `<img>`⇔`![]()` 双方向に保たれるか（レンダリングされるか）を先に確認し、未対応なら最小の画像記法対応から。`clipboardData`/`FileReader` は実機依存のため**実機（拡張機能開発ホスト）で貼り付け→保存→挿入→往復を確認**。`media/` のみなら統合テスト非対象 |
