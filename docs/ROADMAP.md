...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 脚注定義が複数あると往復で空行構成が崩れる | S | `本文[^x]と[^y]です。\n\n[^x]: A\n\n[^y]: B` が往復後に `本文[^x]と[^y]です。\n\n\n[^x]: A\n[^y]: B` になる（本文と定義の間の空行が1つ増え、定義同士の間の空行が消える）。脚注ラベル退避の修正とは無関係で修正前後とも同じ挙動＝既存の欠陥だが、**ラウンドトリップ保持というプロジェクトの必須要件に反する**ため単独項目として記録。`extractFootnoteDefinitions` の `seamIndices` と直列化側の空行復元の噛み合わせが疑わしい。`/local-review` B-1 由来（severity medium、実測で再現確認済み） |
