...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | セル結合(1/4): 列結合（colspan）のMarkdown記法設計とパース/シリアライズ対応 | M | 標準Markdownテーブルにはセル結合の記法が無いため、まずMultiMarkdown方式（結合先のセルの中身を空にすると、直前（左）のセルへ結合されたものとして扱う）を軸に記法を決める。`markdown.js`のテーブルパース（`splitTableRow`/テーブル構築部）で「行内の中身が空のセルが直前セルに続く」パターンを検出し`<td colspan="N">`を生成、`serializeTable`側で`colspan`を見て元の空セル列へ復元できるようにする（列数・`data-sep`との整合に注意）。UI操作（結合コマンド自体）は(3/4)で対応、本タスクは変換ロジックのみ。既存の表関連ユニットテストと同じ流儀で往復テストを追加する。 |
| todo | セル結合(2/4): 行結合（rowspan）のMarkdown記法設計とパース/シリアライズ対応 | M | (1/4)と対になる縦方向の結合。MultiMarkdown方式に倣い、セル内容が`^^`のみの場合は真上のセルへ結合されたものとして扱う案を軸に検討（(1/4)の空セル方式と記法上の役割が被らないことを確認する）。`markdown.js`のテーブル構築・`serializeTable`に`rowspan`の生成・復元を追加。colspanとrowspanが同一セルに同時に付く場合の扱い（対応するか、まず片方ずつのシンプルな組み合わせに限定するか）を実装前に決めておく。 |
| todo | セル結合(3/4): 矩形範囲選択からのセル結合UI（右クリックメニュー/ツールバー） | M | 表の矩形範囲選択機能（`table.js`の`currentCellRange`/`computeCellRange`/`isCellInRange`、選択範囲コピー＆ペーストで確立済み）を流用し、選択中の矩形範囲を1つの結合セル（`colspan`×`rowspan`）へ変換する操作を追加する。矩形でない（L字型等の）選択は結合不可としてエラー表示するガードが必要。(1/4)(2/4)で対応した変換ロジックに乗せて`htmlToMarkdown`まで正しく往復することを確認する。 |
| todo | セル結合(4/4): 結合解除（unmerge）UI | S | 結合済みセル（`colspan`/`rowspan`を持つ`<td>`）を選択して元の個別セル（内容は結合セル1つ分を保持し、残りは空セル）へ戻す操作を追加する。(3/4)の結合UIと対になる操作として同じ右クリックメニュー/ツールバーに配置する。 |
