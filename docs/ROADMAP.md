...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| blocked | 【要実機確認】🖨️ ボタン／コマンドから印刷プレビューが実際に出るか | S | `Ctrl+P` が使えないことは判明したが、**ツールバーの 🖨️ ボタンとコマンドパレット経由はまだ未検証**。VS Code の Webview は sandbox 付き iframe で動くため、`allow-modals` の有無によっては `window.print()` がブロックされ**コンソールに警告だけ出して無反応**になる可能性がある。ここで方針が分岐する: **(a) プレビューが出る** → 印刷ダイアログ方式のまま。上の「専用キーバインド」項目を進めればPDF出力は完成。**(b) 何も起きない** → `window.print()` 方式そのものが使えないため、下の「ヘッドレス変換方式」を`blocked` から `todo` へ格上げして作り直す（(1/4)〜(3/4)で作った印刷用CSSは変換方式でもそのまま流用できる）。人間の実機確認待ちのため `blocked`（ループが着手しても判定できない） |
| todo | 印刷に `Ctrl+P` / `Cmd+P` を使わない導線を整える（専用キーバインドの割り当て） | S | **実機で確認済みの問題**: Webview 内で `Ctrl+P` / `Cmd+P` を押しても VS Code の「クイックオープン（ファイル検索）」が優先され、印刷は起動しない。ツールバーの 🖨️ ボタンとコマンドパレット（`markdown-wysiwyg-editor.exportPdf`）は既に用意してあるので、あとは**キーボードからも呼べる導線**を足す。`package.json` の `contributes.keybindings` へ、VS Code の既定と衝突しないキー（`ctrl+alt+p` / `cmd+alt+p`、または `ctrl+k ctrl+p` のようなコード）を`when: "activeCustomEditorId == 'markdownWysiwyg.editor'"` 付きで登録する。**候補キーが本当に空いているかは実際のVS Codeで確認が要る**（Keyboard Shortcuts で検索して衝突を見る）。`Ctrl+P` 自体を `when` 句で奪う案は、クイックオープンを潰すことになり利用者にとって不意打ちなので採らない。あわせて `docs/commands-and-shortcuts.md` のショートカット表へ追記する |
| blocked | PDFエクスポート: ヘッドレス変換方式（`puppeteer-core` 等）の検討 | M | 印刷ダイアログ方式（上記1〜4）で不足する場合の代替案。HTML→PDF をヘッドレスブラウザで変換するとページ番号・ヘッダー/フッター・目次リンクなどの再現度が上がるが、**依存が重く拡張機能のサイズが増える**（`puppeteer-core` 自体は Chromium を同梱しないが、実行には利用者側のブラウザパスが要る）。Mermaid図のPNG化で確立した「クリーンHTML抽出→変換」の流れを流用できる。**まず印刷方式（1/4〜4/4）を出してから要望を見て判断する**ため、それまでは着手しない＝`blocked`。印刷方式で不足が出た時点で `todo` へ戻す **格上げの条件が具体化した**: 上の「【要実機確認】🖨️ボタンから印刷プレビューが出るか」が**(b) 何も起きない** だった場合、印刷ダイアログ方式は使えないためこの項目が唯一の実現手段になる。その場合は `todo` へ格上げし、(1/4)〜(3/4) で作った `@media print` のスタイルを変換時のHTMLへ流用する形で設計し直す。 |
