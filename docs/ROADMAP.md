...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 全選択への上書き貼り付け（Ctrl+A→Ctrl+V）で先頭に空の殻ブロック（`# ` 等）が残る | M | **既存の失敗テストで検出**（`commands.test.ts` の「全選択への上書き貼り付けで空の殻ブロックが残らない（Ctrl+A→Ctrl+V）」が main 時点で fail）。再現: 既存文書を全選択して別のMarkdownを貼り付けると、貼り付け結果の先頭に旧内容の空の殻（例: 空見出し `# `）が混入する（期待は貼り付け内容のみ）。実測 `# \n\n段落1\n\n\n段落2` / 期待 `段落1\n\n\n段落2`。`commands.js` の `handleMarkdownPaste` が全選択範囲の削除後に空になった始点ブロック（見出し等）を除去しきれていないと思われる。**注意**: このテストは `/evolve` のテストゲート（`npm run test:unit`）を1件失敗させているため、次サイクルで最優先に修正する。まず `handleMarkdownPaste` の選択削除・殻ブロック除去処理を確認 |
