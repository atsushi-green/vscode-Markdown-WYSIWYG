...
| 状態 | 不具合 | サイズ | メモ |
|------|--------|--------|------|
| todo | 触っていない行の表区切り `\|---\|`→`\| --- \|` が勝手に書き換わる（箇条書きマーカーは対応済み） | M | ファイルを開いて**別の行を編集しただけ**で、触っていない表の区切り行 `\|---\|---\|` が `\| --- \| --- \|` に自動で書き換わる（`serializeTable` が常に `\| --- \|` 形式・アライメントコロンも `---` へ潰す）。**箇条書きマーカー `-`/`*` の側は対応済み**（`markdownToHtml` が `data-marker` へ保持→`serializeList` が尊重・`52bef01` の次コミット）。方針は同じ: `markdownToHtml` の表パースで元の区切りバリアント（各列のスペース有無・アライメントコロン `:---`/`---:`/`:---:`）を `<table>`/`<th>` の `data-` 属性に保持し、`serializeTable` がそれを尊重して復元＝往復をバイトレベル冪等にする。回帰テストは「区切りスペース有無・各アライメント」の各バリアントで往復バイト一致を先に追加。**既知の別制約**: 空行なしで異なる箇条書きマーカーが混在（`- a\n* b`）すると現状パーサーが1つのulに束ね先頭マーカーへ統一される（CommonMark では別リスト）。厳密対応するならリスト収集を「マーカーが変わったら別リストで打ち切る」よう直す＝別タスク（`markdown.test.ts` に現状挙動固定テスト有り）。ユーザー報告 |
