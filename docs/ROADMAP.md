...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | 表の矩形選択とコピー＆ペースト(1/3) — マウスドラッグでの矩形範囲選択とハイライト表示 | M | 「表の矩形選択とコピー＆ペースト（Excel等外部アプリとの相互貼り付け対応）」Lを分割した前半（(2/3)(3/3)が依存する基盤）。現状`table.js`は単一セル選択（`selectCell`）のみで、複数セルにまたがる矩形範囲の概念が無い。セルの`mousedown`で始点を記録し`mousemove`中（同一テーブル内）に矩形の対角セルを算出、`mouseup`で確定。選択範囲は行・列インデックスの矩形（開始/終了row・col）として状態保持し、範囲内の各セルへハイライトclass（例: `table-cell-range-selected`）を付与するCSSを追加。範囲算出（開始/終了座標→矩形内セルインデックス列挙）は純粋関数として切り出しユニットテスト可能にする。既存の単一セル選択（クリックのみ・ドラッグ無し）や既存のキーボードナビゲーション（`↑↓←→`/`Tab`）との共存（矩形選択中に既存の単一セル選択状態とどう整合させるか）に注意 |
