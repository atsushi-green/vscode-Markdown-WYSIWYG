...
| 状態 | 機能 | サイズ | メモ |
|------|------|--------|------|
| todo | ブロック数式コピー画像の背景色選択（透過・白・黒）UI配線 | S | 基盤は実装済み: `blockToPngBlob(block, scale, background)` が `background` を受け、既定 `'white'`／`'transparent'`／`'black'` を `buildMathBlockSvgMarkup` の背景rect（透過は無描画）とフォールバック html2canvas 背景に橋渡し済み。**残りは呼び出しUIのみ**（未着手・`copyBlockAsPng` は現状 `blockToPngBlob(block)` を引数無しで呼んでおり常に既定の白）: 数式の右クリックメニュー（`math.js` の `ensureContextMenu`／`setupContextMenu`）は現状「📋 画像としてコピー」の1項目のみで、Mermaidメニューにある背景トグル行（`.mermaid-bg-btn` 相当）が無い。これを追加し `copyBlockAsPng`→`blockToPngBlob` の第3引数へ選択背景を渡す。[[Mermaid図の画像コピー／保存時に背景色を選べる（透過・白・黒）— UI配線]] とパターン共通（`7a1f50e`）。foreignObject taint の実機確認は完了済み（`d6ba754`）のため着手可能 |
