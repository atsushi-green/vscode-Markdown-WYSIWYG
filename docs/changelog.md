...
| 2026-07-15 | `_______` feat: タスクリストのライブ入力対応 | エディタ上で `- [ ]` / `- []` / `-[]`（スペース有無を問わず）を入力した時点でチェックボックスGUIへ即時変換するように。従来はファイル読込時のパーサでしか解釈されず入力中はテキストのまま残っていた。`commands.js` に `convertTaskLists` を追加し、入力イベントの整形フローへ組み込み。 |
