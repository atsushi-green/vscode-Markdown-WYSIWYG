...
| 2026-07-15 | feat: タスクリスト（チェックボックス）の追加 | GFMタスクリスト記法（`- [ ]` / `- [x]`）をチェックボックス付きリストとして表示し、クリックで完了/未完了を切り替え可能に。状態はMarkdownソースへ即時反映。あわせて自動開発ループ用の `docs/ROADMAP.md` と `/evolve` コマンド定義を追加。 |
